...
reg teenobese sports if `wave4flagAll'
reg obese teenobese sports if `wave4flagAll'

*===================================================
* Summary statistics for descriptive stats tables
*===================================================
foreach var of varlist sports white black hispanic other momeddum? dadeddum? liveMoth14 m_liveWithMoth14 liveFath14 m_liveWithFath14 byrdum? byrdum?? enrdum? enrdum?? metrodum? qpwhtdum? gradHS grad4yr attCol empFT regExercise heavyDrunk4 obese {
...
capture lab var sports_100           "Athlete"
...
foreach var in `allvars' {
	...
	if "`test'"=="1" {
		...
		qui ttest `var' if `wave4flagAll', by(sports)
		qui replace holderOverallPflag = (`r(p)'<.05) if _n==`i'
		foreach a in 0 1 {
			qui sum `var' if `wave4flagAll' & sports==`a', d
			local temp: di %3.2f `r(mean)'
	...
	* Sample Sizes
	...
	foreach a in 0 1 {
		qui count if `wave4flagAll' & sports==`a'
		local tempN: di %9.0fc `r(N)'
...
qui listtex holderName holderAthlete0 holderAthlete1 holderOverall in 1/`=`i'' using sumStatsTableFemale.tex, replace delimiter(" & ") end ("   \tabularnewline")
...
* foreach var in sports `vars25labmkt' `outcomes' {
...
* reg FElnWage FEsports `FEvars25labmkt' if `wave4flagAll' & schN_lnWage>5, vce(cluster scid)
* areg  lnWage   sports   `vars25labmkt' if `wave4flagAll', absorb(scid) vce(cluster scid)
...
if !inlist("`dv'","lnWage","inLF","empPT","empFT") {
		...
		regress `dv' sports          if `conder' & `wave4flagAll', vce(cluster scid)
		est sto simple
		regress `dv' sports `vars25' if            `wave4flagAll', vce(cluster scid)
		est sto ols
		* areg    `dv' sports `vars25' if            `wave4flagAll', absorb(scid) vce(cluster scid)
		* est sto arg
		rcr `dv' sports `vars25' if `conder' & `wave4flagAll', cluster(scid) lambda(-1 0)
		forvalues x=-.5(.05).5 {
			if `x'<0  local bounds = "`x' 0"
			if `x'>=0 local bounds = "0 `x'"
			rcr `dv' sports `vars25' if `conder' & `wave4flagAll', cluster(scid) lambda(`bounds')
		}
		rcr `dv' sports `vars25' if `conder' & `wave4flagAll', cluster(scid) lambda(0 1)
		est sto rcr
	}
	else {
		di "Dependent variable is: `dv'"
		regress `dv' sports                        if `conderLM' & `wave4flagAll', vce(cluster scid)
		est sto simple
		regress `dv' sports `vars25labmkt' grad4yr if              `wave4flagAll', vce(cluster scid)
		est sto ols
		* areg    `dv' sports `vars25labmkt' grad4yr if              `wave4flagAll', absorb(scid) vce(cluster scid)
		* est sto arg
		rcr `dv' sports `vars25labmkt' if `conderLM' & `wave4flagAll', cluster(scid) lambda(-1 0)
		forvalues x=-.5(.05).5 {
			if `x'<0  local bounds = "`x' 0"
			if `x'>=0 local bounds = "0 `x'"
			rcr `dv' sports `vars25labmkt' if `conderLM' & `wave4flagAll', cluster(scid) lambda(`bounds')
		}
		rcr `dv' sports `vars25labmkt' if `conderLM' & `wave4flagAll', cluster(scid) lambda(0 1)
	}
	est table simple ols, b(%9.3f) se(%9.3f) stats(N) keep(sports) title("Table: efcts of sports on `dv'")
}
